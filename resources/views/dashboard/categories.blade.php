...
@section('content')
<div class="px-5 py-2">
    <h1 class="fw-bold display-4 text-center">Kelola kategori</h1>
    <div class="py-2"></div>
    @if (session('success'))
    ...
    <div class="d-flex align-items-baseline flex-row">
        ...
        Menampilkan {{ $categories->count() }} hasil for '{{ request('search') }}'
        @else
        Tidak menemukan data dengan kata kunci '{{ request('search') }}'
        @endif
        <a href="{{ route('categories.index') }}" class="btn btn-sm btn-danger ms-2">Hapus</a>
    </div>
    <div class="my-3"></div>
    @endif
    ...
    <div class="collapse" id="collapseFormAdd">
        <div class="card p-3 d-flex flex-column">
            ...
            <form action="{{ route('categories.store') }}" method="POST">
                ...
                <div class="mb-3">
                    <label for="formControlValue" class="form-label">Bobot</label>
                    <input type="text" class="form-control" id="formControlValue" name="value"
                        placeholder="Example: 0.1">
                </div>
                <div class="mb-3">
                    <label class="form-label">Tingkat SAATY</label>
                    <select class="form-select" aria-label="Severity" name="severity">
                        <option value="no_effect">Tidak ada efek</option>
                        <option value="low">Kecil</option>
                        <option value="normal">Netral</option>
                ...
                <tbody>
                    ...
                    <tr style="vertical-align: middle">
                        ...
                        <td align="center">
                            ...
                            <a href="{{ route('categories.destroy', $value->id) }}" class="btn btn-sm btn-danger mx-1"
                            ...
                            <form id="delete-id-{{ $value->id }}" action="{{ route('categories.destroy', $value->id) }}"
